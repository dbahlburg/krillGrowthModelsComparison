...
detMetabolicActivity <- function(dayOfYear = NA, dayLength = NA){
  if(!is.na(dayOfYear)){
    ...
    # adjusted to input file respSeas which includes different dynamics than Figure 5
    # return(ifelse(between(dayOfYear, 1, 122), 0,
    #               ifelse(between(dayOfYear, 122, 167), -0.5/45 * (dayOfYear-122),
    #                      ifelse(between(dayOfYear, 167, 267), -0.5,
    #                             ifelse(between(dayOfYear, 267, 312), -0.5 + 0.5/45 * (dayOfYear-267),
    #                                    ifelse(between(dayOfYear, 312, 365), 0, NA))))))
  }
}
...
# growth model
hofmannLascaraModel <- function(y, time, chlA, iceAlgae, parameters = c(chlorophyllToCarbon = 50, assimilationRate = 0.8)){
  ...
  # calculate amount of assimilated carbon
  ingestionRate <- lookUpTableSizeClassTraits$feedingTime[currentSizeClass] * lookUpTableSizeClassTraits$filtrationRateWaterColumn[currentSizeClass]
  ingestionRateSeaIce <- ingestionRate * 0.05
  ingestedCarbonWater <-  ingestionRate * parameters["chlorophyllToCarbon"] * chlA
  ingestedCarboniceAlgae <- ingestionRateSeaIce * parameters["chlorophyllToCarbon"] * iceAlgae
  ...
  # and 20% of daily ration to avoid unrealistically high growth rates (this modification is not described in the original manuscript but used in
  # the Fortran files)
  netProduction <- ifelse(dailyRation > 20, 0.2 * y, assimilatedCarbon) - carbonRespiration 
  #netProduction <- assimilatedCarbon - carbonRespiration #
  ...
  # find NAs in chlorophyll a - here growth is set to zero
  chlaNAs <- which(is.na(chlA))
  netProduction[chlaNAs] <- 0
  return(netProduction) 
}

#Runge-Kutta-Solver
rk4stepHofmann <- function(state, time, inputChla, inputIceAlgae, dt=1){
  
  k1 <- hofmannLascaraModel(y = state, time = time, chlA = inputChla, iceAlgae = inputIceAlgae)
  k2 <- hofmannLascaraModel(y = state+0.5*k1*dt, time = time + 0.5 * dt, chlA = inputChla, iceAlgae = inputIceAlgae)
  k3 <- hofmannLascaraModel(y = state+0.5*k2*dt, time = time + 0.5 * dt, chlA = inputChla, iceAlgae = inputIceAlgae)
  k4 <- hofmannLascaraModel(y = state+k3*dt, time = time + dt, chlA = inputChla, iceAlgae = inputIceAlgae)
